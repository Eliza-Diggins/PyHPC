...
class SimulationLog:
    ...
    def add(self, category, name, **kwargs):
        ...
        if any([i not in kwargs for i in self._required]):
            raise ValueError(
                "New entry with kwargs = %s failed to contain all required fields %s." % (kwargs, self._required))
        ...
        if "initial_conditions_file" in kwargs and category == "sims":  # - Simulation listed ics
            modlog.debug(
                "Searching for matched ICs for name = %s and ic_path = %s." % (name, kwargs["initial_conditions_file"]))
